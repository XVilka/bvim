...
int save(char *fname, PTR start, PTR end, int flags)
{
	int fd;
	char string[255];
	char *newstr;
	...
	do {
		to_write =
		    (filesize - written) >
		    0xfffe ? 0xfffe : (filesize - written);
...
off_t load(char *fname)
{
	...
	if (block_flag && (filemode == REGULAR)) {
		if (lseek(fd, block_begin, SEEK_SET) < 0) {
			...
			if (filesize == 0) {
				sprintf(string, "\"%s\" No such range: %lu-%lu",
					fname, (unsigned long)block_begin,
...
void d_memmove(PTR dest, PTR src, off_t n)
{
	unsigned len;
	long chunk;
	PTR source;
...
void d_memcpy(PTR dest, PTR src, off_t n)
{
	unsigned len;
	long chunk;
	PTR source;
